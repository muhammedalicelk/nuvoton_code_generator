...
function getAvailablePinsForPart(state, partPinsDb) {
  return new Set(partPinsDb?.parts?.[state.mcu]?.pins || []);
}
...
export function validateConfig(state, pinDb, mcuDb, partPinsDb) {
  ...
  const availablePins = getAvailablePinsForPart(state, partPinsDb);
  ...
  if (state.peripherals.uart0.enabled) {
    ...
    if (!uartSel) errors.push('UART0 pin kombinasyonu geçersiz.');
      ...
      const missing = uartSel.pins.filter((pin) => !availablePins.has(pin.pin)).map((pin) => pin.pin);
      if (missing.length) errors.push(`UART0 pin kombinasyonu bu package için uygun değil: ${missing.join(', ')}`);
  ...
  if (state.peripherals.adc.enabled) {
    ...
    adcSelected.forEach((adcSel)=>{
      if (!availablePins.has(adcSel.pin)) errors.push(`ADC pini bu package için uygun değil: ${adcSel.pin}`);
      else if (usedPins.has(adcSel.pin)) errors.push(`Pin çakışması var: ${adcSel.pin}`);
      else usedPins.set(adcSel.pin, `ADC_CH${adcSel.channel}`);
    });
    if (state.peripherals.adc.trigger === 'stadc') {
      ...
      else if (!availablePins.has(stSel.pin)) errors.push(`ADC ST pini bu package için uygun değil: ${stSel.pin}`);
      else if (usedPins.has(stSel.pin)) errors.push(`Pin çakışması var: ${stSel.pin}`);
      else usedPins.set(stSel.pin, 'ADC_ST');
    }
  }
  ...
  messages.push(`Package pin filtresi aktif: ${availablePins.size} pin`);
